  ...
  return (
    <div class="card text-center rounded mt-4" index={id}>
      <div class="card-body">
        ...
        <button
          class={`btn ${status === "RECEIVED" ? "btn-primary" : ""}  ${
            status == "DELIVERED" ? "btn-success" : ""
          } ${status == "CANCELLED" ? "btn-danger" : ""} ${
            status == "ARRIVING" ? "btn-warning" : ""
          } `}
        >
          {status}
        </button>
